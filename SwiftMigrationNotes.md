...
| ファイル | 理由 |
|---|---|
| `Sources/Core/Bridging-Header-G.h` | ICU C API（`NSString-Unicode.swift`）用（TeX2img ターゲットのみ） |
| `Sources/icu/*.h` | ICU ライブラリの公式 C ヘッダ |

削除済み: `global.h`, `typedef.h`, `UtilityG.h`, `TeX2img_Prefix.pch`, `Bridging-Header-C.h`, `main.m`（プロジェクト参照）
