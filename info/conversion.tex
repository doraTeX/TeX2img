...
\node[output node] (SVG) at ($(PDF with text) + (4,-1)$) {テキスト保持SVG\\（透過）};
\draw[->] (PDF with text) -- node[right,xshift=-8pt,yshift=6pt] {mudraw} (SVG);

\node[relay node] (EPS2) at ($(PDF with text) + (4,-2.5)$) {テキスト形式EPS\\（透過）};
\draw[->] (PDF with text) -- node[below,xshift=-12pt,yshift=-4pt] {pdftops\textsuperscript{*3}} (EPS2);
...
\node[output node] (Filled SVG) at ($(Filled PDF with text) + (-4,-1)$) {テキスト保持SVG\\（非透過）};
\draw[->] (Filled PDF with text) -- node[left,xshift=8pt,yshift=6pt] {mudraw} (Filled SVG);

\node[relay node] (Filled EPS2) at ($(Filled PDF with text) + (-4,-2.5)$) {テキスト形式EPS\\（非透過）};
\draw[->] (Filled PDF with text) -- node[below,xshift=12pt,yshift=-4pt] {pdftops\textsuperscript{*3}} (Filled EPS2);
...
\node[output node] (Animation SVG) at ($(Filled Outlined PDF with margin)!.5!(Outlined PDF without margin)$) {アウトライン化SVG\\アニメーションSVG};
