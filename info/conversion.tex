...
\usepackage[dvipdfm,papersize={58cm,33cm},margin=2cm,noheadfoot]{geometry}
\usepackage{metalogo}
...
\node[relay node] (PDF with text without margin) at ($(PDF) + (-18,-3)$) {テキスト保持PDF\\（余白なし・透過）};
...
\node[output node] (EPS without margin) at ($(PDF) + (6,0)$) {バイナリ形式EPS\\（余白なし・透過）};
\draw[->] (PDF) -- node[below] {Ghostscript\textsuperscript{*1}} (EPS without margin);

\node[output node] (EPS with margin) at ($(EPS without margin) + (4,0)$) {バイナリ形式EPS\\（余白あり・透過）};
...
\node[relay node] (EPS2) at ($(Outlined PDF with margin) + (4,-2)$) {テキスト形式EPS};
\draw[->] (Outlined PDF with margin) -- node[above,xshift=12pt,yshift=4pt] {pdftops\textsuperscript{*3}} (EPS2);
...
\node[output node] (Outlined SVG) at ($(Text EPS) + (-3,2)$) {アウトライン化SVG};
...
\node[output node] (SVG) at ($(PDF with text) + (4,-2)$) {テキスト保持\\SVG\\（透過）};
\draw[->] (PDF with text) -- node[right,xshift=-8pt,yshift=8pt] {mudraw} (SVG);
...
\node[output node] (Filled SVG) at ($(Filled PDF with text) + (-4,-2)$) {テキスト保持\\SVG\\（非透過）};
\draw[->] (Filled PDF with text) -- node[left,xshift=8pt,yshift=8pt] {mudraw} (Filled SVG);
...
\path[->] ($(Outlined PDF without margin.north east) + (-12pt,0pt)$) edge  [loop right,out=135,in=45,distance=35] node[right,xshift=12pt,yshift=2pt] {pdf\TeX で余白付与} ($(Outlined PDF without margin.north east) + (-6pt,0pt)$);
\path[->] ($(Outlined PDF without margin.south east) + (-12pt,0pt)$) edge  [loop right,out=-135,in=-45,distance=35] node[right,xshift=12pt,yshift=-2pt] {Quartz APIで背景塗り} ($(Outlined PDF without margin.south east) + (-6pt,0pt)$);


\node[relay node] (EPS2) at ($(Outlined PDF without margin) + (6,0)$) {EPS};
\draw[->] (Outlined PDF without margin) -- node[above] {pdftops\textsuperscript{*3}} (EPS2);

\node[output node] (EMF) at ($(EPS2) + (0,-2)$) {EMF};
\draw[->] (EPS2) -- node[right,align=left] {eps2emf\\（改造版pstoedit）} (EMF);
