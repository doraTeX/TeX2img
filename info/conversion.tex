...
\path[->] (Single page PDF) edge  [loop right,out=135,in=45,distance=35] node[above,yshift=2pt] {Quartz APIで再保存} (Single page PDF);
...
\draw[->] (PDF) -- node[above] {pdf\TeX} (PDF with text 2);
\draw[->] (PDF with text 2) -- node[right] {Ghostscript\textsuperscript{*1}} (EPS without margin);
